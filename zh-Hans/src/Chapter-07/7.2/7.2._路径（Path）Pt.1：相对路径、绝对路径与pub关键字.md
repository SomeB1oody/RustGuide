...
对于绝对路径，函数`eat_at_restaurant`与`add_to_waitlist`所在的`front_of_house`模块在同一个文件`lib.rs`里，也就是在同一个crate里（`lib.rs`的内容已经隐式地组成了crate这个模块，具体可以参考上一篇文章最后一部分）。所以说绝对路径就是从`crate`开始写起，逐级地写，用`::`分开每级的标识符：
```rust
crate::front_of_house::hosting::add_to_waitlist();
```
