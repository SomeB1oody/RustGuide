...
- 在`eat_at_restaurant`这个函数中先使用相对路径调用了`summer`这个构造器构造了一个实例，把它赋给了可变变量`meal`。而`meal`中的`toast`字段被设为了Rye,`seasonal_fruit`的值则是peaches（构造器中写的）。下一行中，因为`Breakfast`这个结构体是公共的所以`meal.toast`可以直接被更改，这里是改为了Wheat。
