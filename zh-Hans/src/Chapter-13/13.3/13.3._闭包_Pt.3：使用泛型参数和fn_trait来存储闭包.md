...
还记得在 [13.1](../13.1/13.1._闭包_Pt.1：什么是闭包、如何使用闭包.md) 中的例子吗：
...
每个闭包实例都有自己唯一的匿名类型，即使两个闭包签名完全一样，这两个实例仍然是两个类型。所以**存储闭包需要使用泛型以及trait bound**（泛型和trait bound的内容在 [10.4. trait Pt.2](../../Chapter-10/10.4/10.4._trait_Pt.2：trait作为参数和返回类型、trait_bound.md) 中有讲，推荐看看这篇）
...
```
- 把`expensive_closure`作为`Cache`结构体的实例，使用`new`函数把闭包传进去。这里把`expensive_closure`加上`mut`设为可变绑定是因为后文调用时可能会改变`value`这个字段的值。
