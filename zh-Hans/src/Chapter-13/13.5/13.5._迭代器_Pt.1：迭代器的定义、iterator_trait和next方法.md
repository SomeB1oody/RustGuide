...
`Iterator`这个trait仅要求实现一个方法——`next`。`next`方法每次调用都会返回迭代器中的一项，也就是序列中的一个元素，而由于返回类型是`Option`，所以返回的结果会被包裹在`Some`变体里。如果迭代结束，就会返回`None`。
...
```
- `v1`是一个`Vector`，`v1_iter`是`v1`的迭代器，由于下面的操作会被视为修改迭代器的状态，所以得加`mut`关键字声明为可变。
