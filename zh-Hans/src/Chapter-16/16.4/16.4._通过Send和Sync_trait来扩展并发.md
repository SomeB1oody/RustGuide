...
手动实现这些 trait 涉及编写不安全的 Rust 代码。我们将在以后的文章讨论使用不安全的 Rust 代码（关于这部分可以看 [The Rustonomicon](https://doc.rust-lang.org/nomicon/index.html)）；目前，重要的一点是：在构建新的并发类型时，需要仔细考虑 `Send` 和 `Sync` 组成部分，以维护安全保证。
