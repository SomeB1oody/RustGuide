...
在上一篇文章 [16.3. 共享状态的并发](../16.3/16.3._共享状态的并发.md) 我们曾尝试在跨线程的情况下传递 `Rc<T>`，失败了，失败原因就是没有实现 `Send` trait。

Rust 里**几乎所有的类型都实现了 `Send`**：除了原始指针之外，几乎所有的基础类型都实现了 `Send` trait。但 `Rc<T>` 没有实现 `Send`，它只能用于单线程场景。
...
手动实现这些 trait 涉及编写不安全的 Rust 代码。我们将在以后的文章 [19.1. 摆脱安全性限制的 unsafe Rust](../../Chapter-19/19.1/19.1._摆脱安全性限制的unsafe_Rust.md) 讨论使用不安全的 Rust 代码（关于这部分也可以看 [The Rustonomicon](https://doc.rust-lang.org/nomicon/index.html)）；目前，重要的一点是：在构建新的并发类型时，需要仔细考虑 `Send` 和 `Sync` 组成部分，以维护安全保证。
