...
[*The Rust Programming Language*](https://doc.rust-lang.org/book/) 中用这么一段话形容了 Rust 对并发的支持：
...
- 通过调用操作系统的 API 来创建线程，叫做**1:1 模型**，也就是**一个操作系统的线程对应一个语言的线程**。它的优点是需要较小的运行时。

- 语言自己可以实现线程（也叫绿色线程），是**M:N 模型**。也就是 M 个绿色线程对应 N 个系统线程。它需要比较大的运行时。
...
对于 Rust 来说，它尽可能保持几乎没有运行时的状态，这样就能方便地与 C 语言进行交互，并且获得较高的性能。所以**Rust 标准库仅提供 1:1 模型的线程**。

但是由于 Rust 具有良好的底层抽象能力，在社区里也有很多支持**M:N 模型**的第三方包。
...
```
主线程输出完 4 后就要结束了，这时候还有些时间。另外的线程在程序关闭前又输出了两个数。
