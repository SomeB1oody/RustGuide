...
实际上，`0.8.5` 这种版本号写法是一种简写，其完整写法为 `^0.8.5`，表示任何与 `0.8.5` 版本公共 API 兼容的版本都可以。比如某个依赖项的版本是 `1.2`，那就意味着允许升级到任意 `1.2.x` 版本，但不会升级到 `2.0.0` 或更高版本。

**Cargo 会一直使用你指定的依赖项版本，直到你手动指定其他版本。**
...
这个命令会忽略 `Cargo.lock`，通过更新后的注册表来查找满足 `Cargo.toml` 中要求的最新依赖版本，但 `Cargo.toml` 中写下的版本不会改变。举个例子，假如某个依赖项在 `Cargo.toml` 中声明的版本是 `1.2`，`cargo update` 就可以把它升级到最新的 `1.x.x` 版本，但不会升级到 `2.0.0` 或更高版本；同时 `Cargo.toml` 中写下的版本依然是 `1.2`。
...
运行效果如下:
![IDE 中猜数游戏代码及打印出神秘数字的运行结果](./secrete_numberr.png)
