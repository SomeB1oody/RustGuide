...
不过，这个简单例子并不能很好地体现 trait bound 的优势。再看另一个例子。假设我要设计一个新的 `notify1` 函数。它接收两个参数，`Breaking news!` 后面的内容是分别在每个参数上调用 `summarize` 的返回值。
...
显然，前一种函数签名比后一种更好写，也更直观。

实际上，`impl trait` 只是 trait bound 的语法糖，因此它不适合复杂情况也完全可以理解。
...
```
但要记住，初始化 `largest` 时必须把它设为 `&T`，所以需要在 `list[0]` 前面加 `&` 使其成为引用。另外，比较时不能对 `item` 解引用，而需要在 `largest` 前面加 `&`。
