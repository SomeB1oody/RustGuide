# 10.8.1. 方法定义中的生命周期标注
还记得在上一篇文章 [10.7. 生命周期 Pt.3](../10.7/10.7._生命周期_Pt.3：输入输出生命周期与3规则.md) 中所提到的省略生命周期的三条规则吗：
...
方法需要一个结构体，而在结构体上使用生命周期实现方法，它的语法和泛型参数的语法一样（详见文章 [10.7. 生命周期 Pt.3](../10.7/10.7._生命周期_Pt.3：输入输出生命周期与3规则.md)）。
