...
```
当然，读取会可能发生错误，所以它的返回值并不直接就是内容而是`Result`枚举，针对这个枚举，可以使用`expect`方法来解包，`expect`方法的参数是如果发生错误时打印的错误信息（`expect`方法在 [9.2. Result枚举与可恢复的错误 Pt.1](../../Chapter-09/9.2/9.2._Result枚举与可恢复的错误_Pt.1：match、expect和unwrap处理错误.md) 中有详细介绍）。
```rust
let contents = fs::read_to_string(filename)
