...
`panic!`适用于程序本身出现问题时的恐慌，而这里却少参数的输入是程序使用时的问题，针对这种问题，使用`Result`类型来传播错误（这部分的内容详见 [9.2. Result枚举与可恢复的错误 Pt.1](../../Chapter-09/9.2/9.2._Result枚举与可恢复的错误_Pt.1：match、expect和unwrap处理错误.md) & [Pt.2](../../Chapter-09/9.3/9.3._Result枚举与可恢复的错误_Pt.2：传播错误、?运算符与链式调用.md)）才是最优解:
```rust
impl Config {
