...
- `player.into_iter()` 以及传给 `zip` 的 `initial_scores`（也会变成迭代器）会从这两个 `Vector` 中取出拥有所有权的值。使用 `.zip()` 可以创建“`player` 中元素在前、`initial_scores` 中元素在后”的元组序列。如果想交换元素位置，把代码中的两个迭代器互换位置即可。然后再使用 `.collect()` 把这些元组转换成 `HashMap`。因为使用了 `into_iter`，`HashMap` 会获得这些值的所有权（原来的两个 `Vector` 会被消耗掉）。
