...
其实 `Rc<T>` 上也有 `clone` 方法（不是 `Clone` trait 上的 `clone` 方法），其源码与 `Rc::clone` 完全一样，所以在给 `b` 和 `c` 赋值时写 `a.clone()` 也是可以的。但因为这么写可能会被误解为深拷贝——尤其是对新手来说——而它实际上只是增加引用计数，所以不推荐这么写。更好的选择是 `Rc::clone`。
