...
```
- 首先创建了一个链表`List`，其写法在 *[15.1. 使用`Box<T>`来指向堆内存上的数据](../15.1/15.1._使用Box_T_智能指针来指向堆内存上的数据.md)* 中就有详细解释，这里不在阐述
...
这里再次强调，`Rc<T>`引用是不可变的，如果`Rc<T>`允许程序员持有多个可变引用的话就会违反借用规则（详见 [4.4. 引用与借用](../../Chapter-04/4.4/4.4._引用与借用.md)）——**多个指向同一区域的可变引用会导致数据竞争以及数据的不一致。**
