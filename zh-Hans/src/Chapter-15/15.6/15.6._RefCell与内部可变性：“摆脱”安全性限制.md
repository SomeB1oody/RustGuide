...
通常而言，这样的行为会被*借用规则*（详见 [4.4. 引用与借用](../../Chapter-04/4.4/4.4._引用与借用.md)）所禁止，但是为了能够改变数据，内部可变性模式在代码的数据结构里使用了`unsafe`代码来绕过Rust正常的可变性和借用规则。
...
为了了解`RefCell<T>`与`Box<T>`的区别，我们得回顾一下借用规则（详见 [4.4. 引用与借用](../../Chapter-04/4.4/4.4._引用与借用.md)）：
...
与编译时借用规则（详见 [4.4. 引用与借用](../../Chapter-04/4.4/4.4._引用与借用.md)）一样， `RefCell<T>`允许我们在任何时间点拥有许多不可变借用或一个可变借用。
