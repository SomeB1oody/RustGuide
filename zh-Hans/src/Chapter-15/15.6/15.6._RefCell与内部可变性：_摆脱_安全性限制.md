...
- `LimitTracker` 还有一个名为 `set_value` 的方法。其第一个参数是对 `self` 的可变引用 `&mut self`，第二个参数是 `value`，类型为 `usize`。
  方法内部的逻辑很简单。它把 `self.value` 字段除以 `self.max` 字段（两者都转换为 `f64` 以避免丢失精度）得到一个百分比，存入 `percentage_of_max`，并根据 `percentage_of_max` 的大小，使用 `Messenger` trait 的 `send` 方法发送不同的警告。
...
- 首先，在定义 `List` 枚举时，把 `Cons` 关联的 `i32` 用 `RefCell<>` 包裹，以便可以修改该值。外面再用 `Rc<>` 包裹，以便多个所有者可以共享它，其余保持不变。
