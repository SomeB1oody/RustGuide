...
### 9.3.3. `?`与`from`函数Rust提供了`from`函数，它来自`std::connvert::From`这个trait，而它的作用是在错误之间进行转换，将一个错误类型转化为另外一个错误类型，而被`?`所接收的错误，会隐式地被`from`函数处理，`from`会看当前代码所在的函数的返回值的错误类型是什么，然后转换为什么。
