...
Usually, such behavior would be forbidden by the *borrow rules* (see [4.4. Reference and Borrowing](../../Chapter-04/4.4/4.4._Reference_and_Borrowing.md)), but in order to change data, the interior mutability pattern uses `unsafe` code inside the data structure to bypass Rust’s normal mutability and borrowing rules.
...
To understand the difference between `RefCell<T>` and `Box<T>`, we need to revisit the borrow rules (see [4.4. Reference and Borrowing](../../Chapter-04/4.4/4.4._Reference_and_Borrowing.md)):
...
Just like the compile-time borrow rules (see [4.4. Reference and Borrowing](../../Chapter-04/4.4/4.4._Reference_and_Borrowing.md)), `RefCell<T>` allows us to have many immutable borrows or one mutable borrow at any point in time.
