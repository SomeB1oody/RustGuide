...
```
- First we create a linked list `List`; its structure was explained in detail in *15.1. Using `Box<T>` to Point to Data on the Heap*, so we will not repeat it here.
