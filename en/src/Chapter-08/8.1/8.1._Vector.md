...
`pop()`: Removes and returns the last element wrapped in `Option` (covered in [6.2. The Option Enum](../../Chapter-06/6.2/6.2._The_Option_Enum.md)). Returns `None` if empty.
```rust
fn main() {
    let mut v = vec![1, 2, 3];
...
- `v.get(2)`: Uses the `get` method for access. Since it returns an `Option` type, we use `match` (covered in [6.3. The Match Control Flow Operator](../../Chapter-06/6.3/6.3._The_Match_Control_Flow_Operator.md)) to unpack it. If a value exists, it binds to `third` and prints; if not (`None`), it prints "There is no third element."
...
Remember the borrowing rule discussed in [Chapter 4.2. Ownership Rules, Memory, and Allocation](../../Chapter-04/4.2/4.2._Ownership_Rules,_Memory,_and_Allocation.md)? *You cannot have mutable and immutable references in the same scope at the same time.* This rule still applies to `Vector`. Example:
```rust
fn main() {
