...
Remember the enum type introduced in [6.1. Enums](../../Chapter-06/6.1/6.1._Enums.md)? Enum variants can carry **attached data**, and that attached data can be of different types. Most importantly, the **variants all belong to the same enum type**. In other words, **all variants are the same type**, so they can be stored in a `Vector`.
