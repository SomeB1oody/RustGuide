...
- `player.into_iter()` and `initial_scores` (which also becomes an iterator when passed to `zip`) yield owned values from the two `Vector`s. Using `.zip()` creates a sequence of tuples with elements from `player` first and elements from `initial_scores` second. If you want to swap the order of the elements, you can simply swap the two iterators in the code. Then `.collect()` is used to convert the tuples into a `HashMap`. Because `into_iter` is used, the `HashMap` takes ownership of those values (the original `Vector`s are consumed).
