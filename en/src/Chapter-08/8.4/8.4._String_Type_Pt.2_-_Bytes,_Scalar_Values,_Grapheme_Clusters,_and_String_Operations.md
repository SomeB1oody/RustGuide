...
![Devanagari नमस्ते shown as grapheme clusters](./梵文.png)
...
- Indexing is supposed to take constant time, or `O(1)`, but `String` cannot guarantee that: finding the *n*th character requires scanning from the start of the string, because each character can take a variable number of bytes.
