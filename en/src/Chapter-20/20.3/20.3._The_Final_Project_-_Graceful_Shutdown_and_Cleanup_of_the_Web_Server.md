...
1863 |     pub fn join(self) -> Result<T> {
     |                 ^^^^
```
The error message shows that we cannot move the `thread` field out of `worker`, because we only have a mutable reference to each `worker`, but `join` requires ownership of `worker`.
...
```
Use `if let` pattern matching to extract the value when the `worker` is `Some` (using `take` gives us ownership instead of a mutable reference).
...
```
Using `as_ref` avoids an ownership problem: `send` needs ownership, but `execute` only has a reference (`&self`), so we send a reference.
...
```
Remove the last `unwrap` from `job`, and instead use `match` branches: execute `job` for the `Ok` variant, and break for the `Err` variant.
...
You may see different output, because which thread in the pool gets the work is random, but the output should be roughly similar.
