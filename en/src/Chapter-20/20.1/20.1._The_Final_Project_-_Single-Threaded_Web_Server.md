...
Let’s try running this code. In the terminal, run `cargo run` and then load `127.0.0.1:7878` in a web browser. The browser should display an error message such as “connection reset” because the server is not sending anything back yet. But when you look at the terminal, you should see a few messages printed when the browser connects to the server.
