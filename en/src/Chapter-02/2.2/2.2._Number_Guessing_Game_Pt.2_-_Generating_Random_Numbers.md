...
![crates.io page for the rand crate version 0.8.5](./rand_website.png)
...
Finally, print the random number (the use of `println!` was introduced in [2.1 Number Guessing Game Pt.1 - One Guess](../2.1/2.1._Number_Guessing_Game_Pt.1_-_One_Guess.md), so I will not repeat it):
```rust
...
The result is:
![IDE showing guessing game code and output that reveals the secret number](./secrete_numberr.png)
