...
In Chapter 8, we mentioned that one limitation of `Vec`s is that they can store only one element type. We created a workaround in [8.2. Vector + Enum Applications] by defining a `SpreadsheetCell` enum with variants for storing integers, floating-point numbers, and text. This means we can store different types of data in each cell while still having a vector that represents a row of cells. When the items we want to interchange are a fixed set of types that we know when compiling the code, this is a very good solution.
