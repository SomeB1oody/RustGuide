...
One thing to keep in mind when using `Result` for tests: do not use the `should_panic` attribute on tests written with `Result<T, E>` (as discussed in [11.4. Using `should_panic` to Check Panics](../11.4/11.4._Using_should_panic_to_Check_Panics.md)), because tests written with `Result<T, E>` return `Err` instead of panicking directly.
