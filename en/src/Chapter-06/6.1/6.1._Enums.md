...
The concept of methods was introduced in the previous article, [5.3. Methods on Structs](../../Chapter-05/5.3/5.3._Methods_on_Structs.md), so we will not go into too much detail here. Methods are defined with the `impl` keyword, as shown below:
```rust
enum Message {
    Quit,
    Move { x: i32, y: i32 },
