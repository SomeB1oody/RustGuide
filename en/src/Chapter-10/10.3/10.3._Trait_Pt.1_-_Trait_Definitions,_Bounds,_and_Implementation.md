...
- Use the `trait` keyword to define a trait. Methods in a trait can be written as **signatures only** (ending with `;`, no method body), or they can include a **default implementation** (covered later)
- A trait can have multiple methods, and each required method signature is written on its own line and ends with `;`
