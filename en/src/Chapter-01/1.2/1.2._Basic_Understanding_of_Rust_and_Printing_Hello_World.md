...
![RustRover New Project dialog for creating a Rust binary application](./Create_rust.png)
Change the project save path or choose the location of the toolchain according to your needs, then click Create. If the IDE does not recognize the toolchain, check whether Rust has been downloaded and installed. See the installation guide in [1.1. Install Rust](../1.1/1.1._Install_Rust.md).

### Step 2: Write the Code
Because RustRover automatically configures Cargo for new projects (which will be covered in [1.3. Basic Knowledge of Rust Cargo](../1.3/1.3._Basic_Knowledge_of_Rust_Cargo.md)), the project will directly generate `main.rs` and include code for printing Hello World:
![IDE showing Hello World code in main.rs with project tree](./rust_interface.png)
...
![File Explorer and PowerShell after cd into the Rust src folder](./cd.png)
...
![File Explorer after rustc compile, showing main.exe and main.pdb](./rustc.png)

- For **Windows**, enter `.\main.exe` in the terminal; for **Linux/macOS**, enter `./main`. If your program is not named `main`, just replace `main` with your program name. Example output:
```console
$ ./main
Hello World
```
...
- `rustc` is suitable only for simple Rust programs; **complex Rust programs need Cargo** (which will be discussed in [1.3. Basic Knowledge of Rust Cargo](../1.3/1.3._Basic_Knowledge_of_Rust_Cargo.md))
