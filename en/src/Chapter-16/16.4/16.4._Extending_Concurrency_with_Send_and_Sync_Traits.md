...
In the previous article, [16.3. Concurrent Shared State](../16.3/16.3._Concurrent_Shared_State.md), we tried to pass `Rc<T>` across threads and failed because it does not implement the `Send` trait.
...
Manually implementing these traits involves writing unsafe Rust code. We will discuss unsafe Rust in later articles ([19.1. Unsafe Rust - Escaping Safety Restrictions](../../Chapter-19/19.1/19.1._Unsafe_Rust_-_Escaping_Safety_Restrictions.md); for this topic, see also [The Rustonomicon](https://doc.rust-lang.org/nomicon/index.html)); for now, the important point is that when building new concurrent types, the `Send` and `Sync` components need to be considered carefully to preserve safety guarantees.
