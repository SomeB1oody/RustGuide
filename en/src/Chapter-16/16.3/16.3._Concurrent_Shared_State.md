...
error: could not compile `shared-state` (bin "shared-state") due to 1 previous error
```
Look at this part of the error message: `` `Rc<std::sync::Mutex<i32>>` cannot be sent between threads safely ``. `Rc<Mutex<i32>>` cannot be transferred safely between threads. The compiler also tells us why: `` the trait `Send` is not implemented for `Rc<std::sync::Mutex<i32>>` ``. `Rc<Mutex<i32>>` does not implement the `Send` trait (which will be covered in the next article, [16.4. Extending Concurrency with Send and Sync Traits](../16.4/16.4._Extending_Concurrency_with_Send_and_Sync_Traits.md)). Only types that implement `Send` can be transferred safely between threads.
