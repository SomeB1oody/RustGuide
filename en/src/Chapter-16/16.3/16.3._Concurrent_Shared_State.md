...
Why, then, are all primitive types not atomic by default? Why doesn’t the standard library use `Arc<T>` everywhere? Because:
- `Arc<T>` has a performance cost.
- `Arc<T>` and `Rc<T>` have the same API.

Since `Arc<T>` and `Rc<T>` have the same API, the earlier code is easy to change (remember to import `Arc` before using it):
```rust
