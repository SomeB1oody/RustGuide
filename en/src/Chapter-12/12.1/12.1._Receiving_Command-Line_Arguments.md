...
We need a function from Rust’s standard library: `std::env::args()`. This function returns an iterator (covered in [13.5. Iterators Pt. 1](../../Chapter-13/13.5/13.5._Iterators_-_Definitions,_the_Iterator_Trait,_and_the_Next_Method.md)) that yields a series of values. For an iterator, you can use the `collect` method to turn that series of values into a collection, such as a `Vector`.

As discussed in [7.4. Use Pt. 1](../../Chapter-07/7.4/7.4._Keyword_Use_Pt._1_-_Using_Use_and_the_As_Keyword.md), when a function is nested inside more than one module, we usually bring its parent module into scope.
