...
Creating the raw pointer is fine, but *dereferencing* an arbitrary address is undefined behavior. There might or might not be valid data there; the compiler might optimize the access away; or the program might crash—for example with a segmentation fault (exit code 139 / `SIGSEGV`), as in the screenshot above. Behavior can vary by system, compiler, and build settings. You can try it on your own computer.
