...
```
The difference is `.map(ToString::to_string)`, where we pass the `to_string` function directly. The effect is the same as the previous version. By the way, `ToString::to_string` uses the fully qualified syntax discussed in [19.2. Advanced Traits](../19.2/19.2._Advanced_Traits.md).
