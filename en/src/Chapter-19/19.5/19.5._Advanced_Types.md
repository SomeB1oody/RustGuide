...
In Chapter 19.2, we already used the newtype pattern to implement `Display` for `Vec` (more specifically, in 19.2.6, using the newtype pattern to implement an external trait on an external type).

In 19.2.2, Default Generic Parameters and Operator Overloading, we also wrote `Millimeters` and `Meters` structs to store millimeter and meter values separately. Because the two values cannot be added or subtracted directly, this avoids mixing units by mistake.
...
- A new type can hide its internal implementation (as mentioned in 17.1.2, Encapsulation)
...
- Because it is only an *alias*, it is still essentially another `Result<T, E>`, which means we can use any methods that apply to `Result<T, E>` as well as special syntax such as the `?` operator (discussed in 9.3.2, the `?` operator).
...
So what is the never type used for? Let’s use a snippet from the number-guessing game in Chapter 2:
```rust
