...
In [Chapter 7](../../Chapter-07/7.1/7.1._Package,_Crate,_and_Module_Definitions.md), we introduced the `mod` keyword. We use it to organize code into modules. The `pub` keyword introduced there can make modules or methods public so that external code can call them. External code then uses the `use` keyword to bring modules or methods into the current scope.
...
![rustdoc page for the art crate listing kinds and utils modules](./cratedoc.png)
...
![rustdoc page for art crate showing Re-exports of PrimaryColor, SecondaryColor, and mix](./crate_pub_use.png)
