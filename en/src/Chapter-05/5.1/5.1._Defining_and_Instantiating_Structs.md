...
*Unit-like struct*s are called *unit-like structs* because they behave similarly to the unit type `()`. They are used when you need a **type marker** or want to **implement a trait on some type (which you can think of as an interface) without storing any data in the type itself**. This is similar to the empty struct `struct{}` in Go.
