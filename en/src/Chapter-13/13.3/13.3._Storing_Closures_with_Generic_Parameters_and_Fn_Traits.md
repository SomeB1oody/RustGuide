...
Each closure instance has its own unique anonymous type. Even if two closures have exactly the same signature, they are still two different types. So **storing closures requires generics and trait bounds**. The content on generics and trait bounds is covered in [10.4. Trait Pt.2](../../Chapter-10/10.4/10.4._Trait_Pt.2_-_Traits_as_Parameters_and_Return_Types,_Trait_Bounds.md), which is worth a look.
